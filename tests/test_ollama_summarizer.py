from __future__ import annotations

import responses

from personal_brief.models import Item, Pillar
from personal_brief.summarize.ollama import DEFAULT_BASE_URL, OllamaSummarizer


def _item() -> Item:
    return Item(
        pillar=Pillar.FOLLOW,
        source="Sample Blog",
        external_id="1",
        title="A Great Post",
        url="https://example.com/1",
        author="Jane Doe",
        body="Some content about software design.",
    )


@responses.activate
def test_summarize_returns_model_response() -> None:
    responses.add(
        responses.POST,
        f"{DEFAULT_BASE_URL}/api/generate",
        json={"response": "Jane Doe wrote about software design."},
        status=200,
    )
    summarizer = OllamaSummarizer(model="llama3.1")

    result = summarizer.summarize([_item()])

    assert result == "Jane Doe wrote about software design."


def test_summarize_returns_placeholder_for_no_items() -> None:
    summarizer = OllamaSummarizer(model="llama3.1")

    assert summarizer.summarize([]) == "Nothing new today."


@responses.activate
def test_summarize_raises_summarizer_error_on_connection_failure() -> None:
    import requests

    responses.add(
        responses.POST,
        f"{DEFAULT_BASE_URL}/api/generate",
        body=requests.exceptions.ConnectionError("refused"),
    )
    summarizer = OllamaSummarizer(model="llama3.1")

    from personal_brief.summarize import SummarizerError

    try:
        summarizer.summarize([_item()])
        raise AssertionError("expected SummarizerError")
    except SummarizerError as error:
        assert "Ollama" in str(error)
